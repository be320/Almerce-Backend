...
def get_similar_products(user_parameters, N_QUERY_RESULT=5):
    ...
    selected_cluster = kmeans.predict(X=[[normalize_cat(bin_to_dec(
        fixed_user_parameters['category']), maxCat, minCat), fixed_user_parameters['mean_price']]])
    data = encoded_clust[selected_cluster[0]]
    #N_QUERY_RESULT = 5
    nbrs = NearestNeighbors(n_neighbors=N_QUERY_RESULT,
                            algorithm='brute', metric=custom_metric).fit(data)
    ...
    for i in similar_product_indices:
        ...
        id = products[i][0]
        print("id", id)
        query = "select name,image_name,description from toys_shop.products where product_id = '" + \
            str(id)+"';"
...
def custom_metric(X1, X2):
    ...
    age1 = X1[len(X1)-1]
    age2 = X2[len(X2)-1]
    # measure dissamilarity between categories
    category_distance = distance.jaccard(cat1, cat2)
    # normalize mean prices: price 1 and price 2
    ...
    if age2 == 0:
        ...
        dist = math.sqrt(price_distance**2 +
                         category_distance**2 + age_distance**2)
